...
export class GitRepoDO extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path.startsWith("/repo.git/")) {
      ...
      if (subpath === "HEAD") {
        try {
          const head = await this.fs.readFile(".git/HEAD", "utf8") as string;
  ...
  private async handleInfoRefs(service: string): Promise<Response> {
    try {
  ...
  private async handleUploadPack(request: Request): Promise<Response> {
    try {
      ...
      const neededOids = await this.walkObjects(wants, haveSet);
    ...
    try {
      ...
      for (const cmd of commands) {
        if (cmd.newOid === zeroOid) {
          // Delete ref
          this.sql.exec("DELETE FROM refs WHERE name = ?", cmd.refName);
        } else {
          // Create/update ref
          this.sql.exec(
            "INSERT OR REPLACE INTO refs (name, hash) VALUES (?, ?)",
      ...
      if (commands.length > 0) {
        ...
        if (latestCmd) {
          try {
            // Read the pushed commit's tree and extract files to working_tree
            await this.updateWorkingTreeFromCommit(latestCmd.newOid);
          } catch {
  ...
  // ── Helpers for Git Smart HTTP ──

  private getAllRefs(): { name: string; hash: string }[] {
    ...
    // Get all refs
    const refRows = this.sql.exec("SELECT name, hash FROM refs").toArray();
  ...
  private async updateWorkingTreeFromCommit(commitOid: string): Promise<void> {
    // Read commit via isomorphic-git to get tree oid
    const { object: content } = await git.readObject({
      fs: this.fs, dir: "/", oid: commitOid, format: "content",
    });
    ...
    this.sql.exec("DELETE FROM working_tree");
    await this.extractTree(treeOid, "");
  }

  private async extractTree(treeOid: string, prefix: string): Promise<void> {
    const { object: content } = await git.readObject({
      fs: this.fs, dir: "/", oid: treeOid, format: "content",
    });
    ...
    while (pos < treeData.length) {
      ...
      if (mode === "40000") {
        await this.extractTree(sha, fullPath);
      } else {
        try {
          const { object: blobContent } = await git.readObject({
            fs: this.fs, dir: "/", oid: sha, format: "content",
          });
          const now = Date.now();
          this.sql.exec(
            "INSERT OR REPLACE INTO working_tree (path, content, mtime) VALUES (?, ?, ?)",
            fullPath,
            blobContent as Uint8Array,
            now
          );
        } catch {
...
function sideBandPacket(band: number, data: Uint8Array): Uint8Array {
...
export default {
  async fetch(
    ...
    if (isGitSmartHTTPRequest(url)) {
      ...
      if (!checkBasicAuth(request, env.API_KEY)) {
