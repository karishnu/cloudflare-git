  ...
  try {
    ...
    if (offset < body.length) {
      ...
      if (packData.length > 0 && decoder.decode(packData.slice(0, 4)) === "PACK") {
        ...
        await ctx.fs.writeFile(packPath, packData);
...
export async function handleHead(ctx: HandlerContext): Promise<Response> {
  try {
    const head = await ctx.fs.readFile(".git/HEAD", "utf8") as string;
...
function getAllRefs(ctx: HandlerContext): { name: string; hash: string }[] {
...
async function updateWorkingTreeFromCommit(
  ...
  const { object: content } = await git.readObject({
    fs: ctx.fs, dir: "/", oid: commitOid, format: "content",
  });
  ...
  ctx.sql.exec("DELETE FROM working_tree");
  await extractTree(ctx, treeOid, "");
}
...
async function extractTree(
  ...
  const { object: content } = await git.readObject({
    fs: ctx.fs, dir: "/", oid: treeOid, format: "content",
  });
  ...
  while (pos < treeData.length) {
    ...
    if (mode === "40000") {
      await extractTree(ctx, sha, fullPath);
    } else {
      try {
        const { object: blobContent } = await git.readObject({
          fs: ctx.fs, dir: "/", oid: sha, format: "content",
        });
        const now = Date.now();
        ctx.sql.exec(
